...
from helpers.artwork import (
    crop_to_square_jpeg_async,
    embed_metadata_and_artwork_to_mp3,
    fetch_deezer_artwork_async,
    fetch_itunes_artwork_async,
    resolve_album_art,
)


@pytest.mark.asyncio
async def test_fetch_itunes_artwork_success(tmp_path):
    ...
    with patch("httpx.AsyncClient", return_value=mock_client_ctx), \
         patch("helpers.artwork.crop_to_square_jpeg_async", new_callable=AsyncMock, return_value=str(out_file)):
        res = await fetch_itunes_artwork_async("Kodak Black Tunnel Vision", out_file)
...
@pytest.mark.asyncio
async def test_fetch_itunes_artwork_empty_results(tmp_path):
    ...
    with patch("httpx.AsyncClient", return_value=mock_client_ctx):
        res = await fetch_itunes_artwork_async("NonExistentTrackXYZ", out_file)
        assert res is None


@pytest.mark.asyncio
async def test_fetch_deezer_artwork_success(tmp_path):
    ...
    with patch("httpx.AsyncClient", return_value=mock_client_ctx), \
         patch("helpers.artwork.crop_to_square_jpeg_async", new_callable=AsyncMock, return_value=str(out_file)):
        res = await fetch_deezer_artwork_async("Kodak Black Tunnel Vision", out_file)
...
def test_resolve_album_art_itunes_primary(tmp_path):
    ...
    with patch("helpers.artwork.fetch_itunes_artwork_async", new_callable=AsyncMock, return_value={"artwork_path": str(out_file)}):
...
def test_resolve_album_art_fallback_to_youtube_crop(tmp_path):
    ...
    with patch("helpers.artwork.fetch_itunes_artwork_async", new_callable=AsyncMock, return_value=None), \
         patch("helpers.artwork.fetch_deezer_artwork_async", new_callable=AsyncMock, return_value=None), \
         patch("helpers.artwork.crop_to_square_jpeg_async", new_callable=AsyncMock, return_value=str(out_file)):
        result = resolve_album_art(
...
def test_resolve_album_art_concurrent_deezer_fallback(tmp_path):
    ...
    with patch("helpers.artwork.fetch_itunes_artwork_async", new_callable=AsyncMock, return_value=None), \
         patch("helpers.artwork.fetch_deezer_artwork_async", new_callable=AsyncMock, return_value={"artwork_path": str(out_file)}):
        result = resolve_album_art(
            artist="Kodak Black",
            song_name="Tunnel Vision",
...
@pytest.mark.asyncio
async def test_crop_to_square_jpeg_from_url(tmp_path):
    ...
    with patch("httpx.AsyncClient", return_value=mock_client_ctx), \
        ...
        res = await crop_to_square_jpeg_async("https://example.com/cover.jpg", out_file)
